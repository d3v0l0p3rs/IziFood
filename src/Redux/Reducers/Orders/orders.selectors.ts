...
export const filterPendingOrders = createSelector([ordersSelector], (orders) => {
  return orders?.filter((order) => order.currentState === OrdersCurrentState.ON_HOLD)
})

export const filterCookingOrders = createSelector([ordersSelector], (orders) => {
  return orders?.filter((order) => order.currentState === OrdersCurrentState.IN_PROGRESS)
})

export const filterReadyOrders = createSelector([ordersSelector], (orders) => {
