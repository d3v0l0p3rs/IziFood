...
export default function CustomDialog(props: CustomDialogProps): JSX.Element {
  ...
  const currentCards = useSelector((state: rootState) => state.ordersReducer.orders)

  const { register, handleSubmit, watch, errors } = useForm<CardProps>({ mode: "all" })
  const onSubmit = handleSubmit((data) => {
    data.currentState = "on_hold"
    data.id = 4
    const newOrders = [...currentCards, data]
  ...
  return (
    <Dialog open={props.show}>
      ...
      <form onSubmit={onSubmit}>
        ...
        <DialogActions>
          <button onClick={() => props.negativeAction()} type="button">
            Cancelar
          </button>
